...
import { Form, Row, Col, Button, Cascader, DatePicker, Input, Modal, notification } from 'antd'
...
class OwnMailFilter extends Component {
  ...
  handleSearch = (e) => {
    ...
    this.props.form.validateFields((err, fieldsValue) => {
      if (err && err.products.errors[0].message === '请选择产品与服务器') {
        this.openNotificationWithIcon('warning', err.products.errors[0].message)
      }
      if (!err) {
        ...
        if (fieldsValue.allianceName) {
          value.allianceName = fieldsValue.allianceName
        }
              ...
              <Row gutter={20}>
                ...
                <Col {...ColProps} xl={{ span: 5 }} md={{ span: 6 }}>
                  {getFieldDecorator('allianceName', {
                    rules: [{ required: false, message: '请输入联盟名称' }]
                  })(
                    <Input
                      placeholder='请输入联盟名称'
                    />
